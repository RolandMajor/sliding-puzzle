...
class PuzzleStateTest {
    ...
    private PuzzleState state2;
    private PuzzleState state3;
    private PuzzleState state4;

    @BeforeEach
    void setUp() {
        state1 = new PuzzleState(); // the original initial state

        state2 = new PuzzleState(new Position(1, 1),
                new Position(1, 1),
                new Position(1, 1),
                new Position(1, 2)); // a goal state

        state3 = new PuzzleState(new Position(1, 1),
                new Position(2, 0),
                new Position(1, 1),
                new Position(0, 2)); // a non-goal state

        state4 = new PuzzleState(new Position(0, 0),
                new Position(1, 0),
                new Position(0, 1),
    ...
    //TODO: change other move_statex_xxxx() tests like this one
    @Test
    void move_state1_right() {
        state1.move(Direction.RIGHT);
        assertEquals(new Position(0,1), state1.getPosition(0));
        assertEquals(new Position(2,0), state1.getPosition(1));
        assertEquals(new Position(1,1), state1.getPosition(2));
        assertEquals(new Position(0,2), state1.getPosition(3));
    }
